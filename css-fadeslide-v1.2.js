...
$.fn.fadeIn = function(args, callback) {
	...
	var cssTransition = function($el) {
		var transitionEnd = getEventName() + '.fadeIn';
		$el.removeTransition().css({
			'opacity' : '0',
			'display' : 'block'
		}).setFadeTransition(_.duration, _.easing).on(transitionEnd, function(e){
			$el.off(transitionEnd).removeAttr('style').show();
			setTimeout(function() { if (typeof(_.callback) == "function") _.callback(); }, 0);
		}).css('opacity', '1');
	};
...
$.fn.fadeOut = function(args, callback) {
	...
	======================================================= */
	var cssTransition = function($el) {
		var transitionEnd = getEventName() + '.fadeOut';
...
$.fn.slideUp = function(args, callback) {
	...
	var cssTransition = function($el) {
		...
		var transitionEnd = getEventName() + '.slideUp';

		$el.removeTransition().css({
			'height'   : $el.outerHeight(),
			'overflow' : 'hidden'
		}).setSlideTransition(_.duration, _.easing).on(transitionEnd, function(e){
...
$.fn.slideDown = function(args, callback) {
	...
	var cssTransition = function($el) {
		...
		var transitionEnd = getEventName() + '.slideDown';

		$el.removeAttr('style').hide().removeTransition();
		...
		$el.css({
			...
			$el.off(getEventName()).removeAttr('style').show().redraw();
			setTimeout(function() { if (typeof(_.callback) == "function") _.callback(); }, 0);
		}).css({
			'height'         : height,
